...
Event lifecycle service owning scheduled and live presentation sessions, attendance, presenter dashboard state, and the orchestration of bundle and presentation references.

## Repository layout

```
backend/
    src/Smolla.Events.Host/   ASP.NET Core web host
    tests/Smolla.Events.Tests/ xUnit unit + integration tests
```
...
dotnet build
dotnet test
dotnet run --project src/Smolla.Events.Host
```
